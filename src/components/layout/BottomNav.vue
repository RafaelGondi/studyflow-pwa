  ...
  <nav class="fixed bottom-0 left-0 right-0 z-50 bg-app-card border-t border-app-border pb-safe">
    <div class="flex items-center justify-around h-16 max-w-lg mx-auto px-2">
      <RouterLink
        ...
        :key="tab.to"
        :to="tab.to"
        class="flex flex-col items-center gap-1 px-4 py-2 rounded-md transition-all duration-200 group"
        :class="isActive(tab.to) ? 'text-accent' : 'text-muted hover:text-secondary'"
      >
        <div
          class="relative flex items-center justify-center w-10 h-8 rounded-md transition-all duration-200"
          :class="isActive(tab.to) ? 'bg-accent/15' : 'group-hover:bg-app-elevated'"
        >
          <component :is="tab.icon" class="w-5 h-5" :stroke-width="isActive(tab.to) ? 2.5 : 1.8" />
          <span
            v-if="tab.badge"
            class="absolute -top-1 -right-1 min-w-[1.1rem] h-[1.1rem] px-1 flex items-center justify-center rounded-full bg-accent text-white text-[10px] font-semibold"
          >
            {{ tab.badge }}
          </span>
        </div>
